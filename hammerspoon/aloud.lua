-- aloud — read any selected text aloud, in any app · https://github.com/[you]/aloud
--
...
--
-- Install: copy to ~/.hammerspoon/aloud.lua and add require("aloud") to init.lua

-- ============================== configuration ==============================

local ALOUD = "/opt/homebrew/bin/aloud"
if not hs.fs.attributes(ALOUD) then ALOUD = "/usr/local/bin/aloud" end
...
local T = {
  ...
  ready = es and "aloud listo: ⌥⌘L lee · ⌥⌘K para · ⌥⌘H historial"
             or "aloud ready: ⌥⌘L read · ⌥⌘K stop · ⌥⌘H history",
}

-- ============================== state ======================================

local HIST = os.getenv("HOME") .. "/Library/Application Support/Aloud/history.jsonl"
...
local speed = hs.settings.get("aloud.speed") or hs.settings.get("lector.velocidad") or 1.3
local lang = hs.settings.get("aloud.lang") or hs.settings.get("lector.idioma") or "a"
local voice = hs.settings.get("aloud.voice") or hs.settings.get("lector.voz") or nil
...
local function aloudCmd(...)
  hs.task.new(ALOUD, nil, { ... }):start()
end
...
local function setSpeed(v)
  ...
  hs.settings.set("aloud.speed", speed)
  aloudCmd("--speed-live", tostring(speed))
...
-- launch aloud with args, managing the HUD and the active task
  ...
  for _, opt in ipairs(VOICE_MENU) do
    table.insert(voiceMenu, {
      ...
      fn = function()
        ...
        hs.settings.set("aloud.lang", lang)
        hs.settings.set("aloud.voice", voice)
      end,
    })
  end
...
-- console access: hs -c "aloud.read('hello')" / hs -c "aloud.stop()"
aloud = { read = startReading, stop = stopReading, history = history,
